...
const HeaderItemMenu = (props: typeof MAIN_HEADER[0]) => {
  ...
  const handlePathDetect = (href?: string | null) => {
    if (!href) return;
  ...
  return (
    <Menu.Root>
      <Menu.Trigger class="group">
        <Show
          when={props.href}
          fallback={
            <div onClick={() => handlePathDetect(props.href)} class="flex items-center gap-1 cursor-pointer">
              <Typography class="text-neutral-300">{props.name}</Typography>
      ...
      <Menu.Positioner>
        <Menu.Content class="rounded-md z-[1000] bg-neutral-950 w-[200px]">
