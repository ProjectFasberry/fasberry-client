...
const mapPrefix = (prefix: string) => (folders: string[]) => folders.map((folder) => `${prefix}/${folder}`);

const folders = (
	await Promise.all([readdir("apps").then(mapPrefix("apps")), readdir("packages").then(mapPrefix("packages"))])
).flat();
